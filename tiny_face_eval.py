...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
  ...
  #filenames.extend(glob.glob(os.path.join(data_dir, "*count_*.png")))

  dirs = [f for f in glob.glob(data_dir + '/*/')]
  dirs.sort()
  images = []

  for bla in dirs:
    images.append([f for f in glob.glob(bla + '/*_y.png')])

  images.sort()
  images = [item for sublist in images for item in sublist]


  #filenames.extend(glob.glob(os.path.join(data_dir, "*.JPG")))
...
def main():
  ...
  argparse.add_argument('--line_width', type=int, help='Line width of bounding boxes(0: auto).', default=0)
  ...
  assert os.path.exists(input), "data directory: " + input + " not found."
  assert os.path.exists(output), "output directory: " + output + " not found."
  assert args.line_width >= 0, "line_width should be >= 0."

  with tf.Graph().as_default():
    evaluate(
      weight_file_path=args.weight_file_path, data_dir=input, output_dir=output,
